...
def index():
    if 'user' in session:  # 判斷是否有使用者已經登入
        user = session['user']
        combined_prompt = ""  # 初始化 combined_prompt 變數
        text = ""  # 初始化 text 變數
        ...
        if request.method == "POST":
            ...
            return render_template("index.html", lines=lines, user=user, combined_prompt=combined_prompt, text=text)  # 將結果傳遞給模板並返回

        return render_template("index.html", user=user, combined_prompt=combined_prompt, text=text)
